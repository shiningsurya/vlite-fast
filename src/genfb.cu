...
static float EARLY = 3.0f;

static double UTCEPOCH = 596980800.0;
static double MJD      = 47496.5;
...
int main (int argc, char *argv[])
{
  ...
  mc_control_sock = open_mc_socket (mc_vlitegrp, MC_READER_PORT,
      (char*)"Control Socket [GENFB]", NULL, mlog);
  ...
  // 2hrs -> 100 observations
  for (int iobs = 0; iobs < 300; iobs++)
  {
    ...
    // -- this is where we get epoch and injection parameters
